*! version 2.0.1 23sep2021 daniel klein
program usesome
    version 10.1
...
program ds_findname
    /*
        ...
        we do not allow options that might be added
         to -ds- and/or -findname- after 13may2021
        
        we also do not allow any reporting options
        
        modern options restrict <varspec>, i. e., 
         select the intersection of <varspec> and r(varlist)
    */
...
program old_syntax
    /*
        ...
        are old syntax, where has() and not() add to <varspec>
        
        documented overlaps do not trigger the old syntax warning
        
        note: all of the above are technically considered old syntax
...
void describe_dta(`usesome_info' info)
{
    /*
        ...
        check whether Stata's -describe- can do the job
    ...
    */
    ...
    /*
        ...
        we support dta-formats:
            ...
            119 := Stata release 15--17 (more than 32,767 variables)
            118 := Stata release 14--17
        ...
        in case of error, the (sub)routines
            ...
            - exit with the respective Stata error    
        
        note: future changes to the dta-format 118/119
              might require changes to the subroutines
    */
...
void get_ds_fmt(`usesome_info' info)
{
    ...
    if ( anyof((117, 118, 119), info.ds_fmt) ) return
...
void get_varlist(`usesome_info' info)
{
    ...
    info.nvar = hexread(info, ((info.ds_fmt==119) ? 4 : 2))
...
void assert_marker(`usesome_info' info, string scalar marker)
{   
    string scalar bad
    
    if ((bad=fread(info.fh, strlen(marker))) == marker) return
...
real scalar get_nlist_el(transmorphic scalar el, `usesome_info' info)
{
    real scalar isreal
    real scalar realel
...
void ds_findname(`usesome_info' info)
{
    ...
    /*
        the calls to st_ds_findname() below assure that
         even on errors, we return in r()
...
2.0.0 13may2021 complete rewrite
